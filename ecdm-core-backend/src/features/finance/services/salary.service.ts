...
export const getById = async (id: string) => {
  const salary = await Salary.findById(id).populate('employeeId', 'firstName lastName email department position');
...
export const generateEmployeeSalary = async (input: SalaryGenerationInput): Promise<any> => {
  ...
  if (existingSalary) {
    throw new AppError(`Salary already exists for this employee in ${input.month}/${input.year}`, 400);
  }
  
  const summary = await getAttendanceSummary(new Types.ObjectId(input.employeeId), input.month, input.year);
...
export const previewSalaries = async (month: number, year: number): Promise<PreviewResult> => {
  ...
  if (year < 2020) {
    throw new AppError('Year must be 2020 or later', 400);
  }
  ...
  for (const employee of employees) {
    ...
    if (existingSalary) {
      ...
      const grossSalary = existingSalary.basicSalary + existingSalary.allowances +
                          existingSalary.overtime + existingSalary.bonuses;
      const totalDeductions = existingSalary.absenceDeduction + existingSalary.tax +
                               existingSalary.insurance + existingSalary.otherDeductions;
    ...
    const summary = await getAttendanceSummary(new Types.ObjectId(employee._id), month, year);
    const basicSalary = employee.salary || 0;
    const absenceDeduction = calculateAbsenceDeduction(basicSalary, summary);
