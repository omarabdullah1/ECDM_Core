...
for (const file of files) {
  ...
  // 100% safe literal replacement
  content = content.split('onClick={() => setInternalPreviewMode(false)}').join('key="btn-edit" onClick={(e) => { e.preventDefault(); setInternalPreviewMode(false); }}');
  content = content.split('onClick={() => setIsReadOnly(false)}').join('key="btn-edit" onClick={(e) => { e.preventDefault(); setIsReadOnly(false); }}');

  // Prevent duplicate keys
  content = content.split('key="btn-edit" key="btn-edit"').join('key="btn-edit"');
...
console.log('Total files safely patched: ' + changedCount);
