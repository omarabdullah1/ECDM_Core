...
interface SalesLead { _id: string; fullName: string; phone: string; platform: string; orderType: string; status: string; notes?: string; }
...
const PLATFORMS = ['Facebook', 'Instagram', 'TikTok', 'Snapchat', 'GoogleAds', 'Website', 'Referral', 'WhatsApp', 'Cold Call', 'Exhibition', 'Other'];
const ORDER_TYPES = ['Maintenance', 'Installation', 'Spare Parts', 'Inspection', 'Other'];
const STATUSES = ['New', 'In Progress', 'Quoted', 'Won', 'Lost'];
const blank = { fullName: '', phone: '', platform: '', orderType: '', status: 'New', notes: '' };
...
export default function SalesLeadsPage() {
  ...
  const openE = (r: SalesLead) => { setEditing(r); setForm({ fullName: r.fullName, phone: r.phone, platform: r.platform, orderType: r.orderType, status: r.status, notes: r.notes || '' }); setError(''); setModal(true); };
  const save = async (ev: React.FormEvent) => {
    ev.preventDefault(); setSaving(true); setError('');
    const pl: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(form)) { if (v !== '') pl[k] = v; }
    try {
      if (editing) await api.put(`/sales/leads/${editing._id}`, pl);
      else await api.post('/sales/leads', pl);
      setModal(false); fetch_();
    } catch (e: unknown) { setError((e as { response?: { data?: { message?: string } } }).response?.data?.message || 'Failed'); }
    setSaving(false);
  };
  const del = async () => { if (!delId) return; try { await api.delete(`/sales/leads/${delId}`); fetch_(); } catch { } setDelId(null); };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3"><Users className="h-7 w-7 text-[hsl(var(--primary))]" /><h1 className="text-2xl font-bold">Sales Leads</h1></div>
        ...
        {loading ? <div className="p-8 text-center text-[hsl(var(--muted-foreground))]">Loading…</div> : (
          <table className="w-full text-sm">
            <thead className="border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]/30">
              <tr>{['Name','Phone','Platform','Order Type','Status','Actions'].map(h => <th key={h} className="px-4 py-3 text-left font-semibold">{h}</th>)}</tr>
            </thead>
            <tbody>
              {rows.map(r => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-2xl">
            ...
            <form onSubmit={save} className="space-y-4">
              ...
              <select required value={form.platform} onChange={u('platform')} className={iCls}><option value="">Select Platform</option>{PLATFORMS.map(s => <option key={s} value={s}>{s}</option>)}</select>
              <select required value={form.orderType} onChange={u('orderType')} className={iCls}><option value="">Select Order Type</option>{ORDER_TYPES.map(s => <option key={s} value={s}>{s}</option>)}</select>
