...
import { Package, Plus, Edit2, Trash2, X, ChevronLeft, ChevronRight } from 'lucide-react';

interface CustomerOrder { _id: string; status: string; customer?: { name: string }; salesOrder?: { quotationStatus: string }; createdAt: string; }
...
const STATUSES = ['Pending', 'Scheduled', 'InProgress', 'Completed', 'Cancelled'];
const blank = { customer: '', salesOrder: '', status: 'Pending', notes: '' };
...
export default function CustomerOrdersPage() {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get('/customer/orders', { params: p });
  ...
  const openE = (r: CustomerOrder) => { setEditing(r); setForm({ customer: '', salesOrder: '', status: r.status, notes: '' }); setError(''); setModal(true); };
  const save = async (ev: React.FormEvent) => {
    ev.preventDefault(); setSaving(true); setError('');
    const pl: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(form)) { if (v !== '') pl[k] = v; }
    try {
      if (editing) await api.put(`/customer/orders/${editing._id}`, pl);
      else await api.post('/customer/orders', pl);
      setModal(false); fetch_();
    } catch (e: unknown) { setError((e as { response?: { data?: { message?: string } } }).response?.data?.message || 'Failed'); }
    setSaving(false);
  };
  const del = async () => { if (!delId) return; try { await api.delete(`/customer/orders/${delId}`); fetch_(); } catch { } setDelId(null); };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3"><Package className="h-7 w-7 text-[hsl(var(--primary))]" /><h1 className="text-2xl font-bold">'Customer Orders'</h1></div>
        ...
        {loading ? <div className="p-8 text-center text-[hsl(var(--muted-foreground))]">Loading…</div> : (
          <table className="w-full text-sm">
            <thead className="border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]/30">
              <tr>{['Customer','Sales Order','Status','Date','Actions'].map(h => <th key={h} className="px-4 py-3 text-left font-semibold">{h}</th>)}</tr>
            </thead>
            <tbody>
              {rows.map(r => (
                <tr key={r._id} className="border-b border-[hsl(var(--border))]/50 hover:bg-[hsl(var(--muted))]/20">
                  ...
                  <td className="px-4 py-3">{r.salesOrder?.quotationStatus || '—'}</td>
              ...
              {!rows.length && <tr><td colSpan={5} className="px-4 py-8 text-center text-[hsl(var(--muted-foreground))]">No orders found.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-6"><h2 className="text-lg font-bold">{editing ? 'Edit Order' : 'New Customer Order'}</h2><button onClick={() => setModal(false)}><X className="h-5 w-5" /></button></div>
            <form onSubmit={save} className="space-y-4">
              {!editing && <input required placeholder="Customer ID" value={form.customer} onChange={u('customer')} className={iCls} />}
              {!editing && <input required placeholder="Sales Order ID" value={form.salesOrder} onChange={u('salesOrder')} className={iCls} />}
