    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold">{t.pages.salesLeads.title}</h1><p className="text-sm text-[hsl(var(--muted-foreground))]">{total} {t.common.total}</p></div>
        </div>
        <button onClick={openC} className="flex items-center gap-2 bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] px-4 py-2 rounded-xl text-sm font-medium hover:opacity-90"><Plus size={16}/>{t.pages.salesLeads.addBtn}</button>
      </div>
      <div className="flex gap-3 flex-wrap">
        <select value={fPlatform} onChange={e=>{setFPlatform(e.target.value);setPage(1);}} className={iCls+' !w-44'}><option value="">{t.common.allPlatforms}</option>{PLATFORMS.map(p=><option key={p}>{p}</option>)}</select>
        <select value={fType} onChange={e=>{setFType(e.target.value);setPage(1);}} className={iCls+' !w-56'}><option value="">{t.pages.salesLeads.allOrderTypes}</option>{ORDER_TYPES.map(o=><option key={o}>{o}</option>)}</select>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]/30">
              <tr>{[t.common.customer, t.common.salesPerson, t.common.platform, t.common.orderType, t.common.followUp, t.common.date, t.common.notes, t.common.actions].map(h=><th key={h} className="px-4 py-3 text-start font-medium text-[hsl(var(--muted-foreground))]">{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading?(<tr><td colSpan={8} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">{t.common.loading}</td></tr>)
              :rows.length===0?(<tr><td colSpan={8} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">{t.pages.salesLeads.emptyState}</td></tr>)
              :rows.map(r=>(
      ...
      {modal&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between p-6 border-b border-[hsl(var(--border))]"><h2 className="text-lg font-semibold">{editing ? t.common.edit : t.common.new} — {t.pages.salesLeads.title}</h2><button onClick={()=>setModal(false)} className="p-2 hover:bg-[hsl(var(--muted))] rounded-lg"><X size={18}/></button></div>
          <form onSubmit={save} className="p-6 space-y-4">
            ...
            <div><label className="block text-sm font-medium mb-1.5">{t.common.salesPerson} *</label>
              <select className={iCls} value={form.salesPerson} onChange={u('salesPerson')} required><option value="">{t.pages.salesLeads.selectSalesPerson}</option>{users.map(u2=><option key={u2._id} value={u2._id}>{u2.firstName} {u2.lastName}</option>)}</select></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium mb-1.5">{t.common.platform} *</label><select className={iCls} value={form.platform} onChange={u('platform')} required><option value="">{t.common.select}</option>{PLATFORMS.map(p=><option key={p}>{p}</option>)}</select></div>
              <div><label className="block text-sm font-medium mb-1.5">{t.common.orderType} *</label><select className={iCls} value={form.typeOfOrder} onChange={u('typeOfOrder')} required><option value="">{t.common.select}</option>{ORDER_TYPES.map(o=><option key={o}>{o}</option>)}</select></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium mb-1.5">{t.pages.salesLeads.followUpRequired}</label><select className={iCls} value={form.followUpRequired} onChange={u('followUpRequired')}><option value="false">{t.common.no}</option><option value="true">{t.common.yes}</option></select></div>
              <div><label className="block text-sm font-medium mb-1.5">{t.pages.salesLeads.followUpDate}</label><input className={iCls} type="date" value={form.followUpDate} onChange={u('followUpDate')}/></div>
            </div>
      ...
      {delId&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] w-full max-w-sm p-6 space-y-4">
          <h2 className="text-lg font-semibold">{t.pages.salesLeads.deleteTitle}</h2>
          <p className="text-sm text-[hsl(var(--muted-foreground))]">{t.pages.salesLeads.deleteMsg}</p>
