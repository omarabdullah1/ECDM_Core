    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold">Work Orders</h1><p className="text-sm text-[hsl(var(--muted-foreground))]">{total} total</p></div>
        </div>
        <button onClick={openC} className="flex items-center gap-2 bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] px-4 py-2 rounded-xl text-sm font-medium hover:opacity-90"><Plus size={16}/>New Work Order</button>
      </div>
      <div className="flex gap-3 flex-wrap">
        ...
        <select value={fPunct} onChange={e=>{setFPunct(e.target.value);setPage(1);}} className={iCls+' !w-40'}><option value="">Punctuality</option>{PUNCTUALITIES.map(p=><option key={p}>{p}</option>)}</select>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]/30">
              <tr>{['Customer','Engineer','Type','Issue','Site Visit','Punctuality','Notes','Actions'].map(h=><th key={h} className="px-4 py-3 text-left font-medium text-[hsl(var(--muted-foreground))]">{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading?(<tr><td colSpan={8} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">Loading…</td></tr>)
              :rows.length===0?(<tr><td colSpan={8} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">No work orders found</td></tr>)
              :rows.map(r=>(
      ...
      {modal&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] w-full max-w-xl max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between p-6 border-b border-[hsl(var(--border))]"><h2 className="text-lg font-semibold">{editing?'Edit':'New'} Work Order</h2><button onClick={()=>setModal(false)} className="p-2 hover:bg-[hsl(var(--muted))] rounded-lg"><X size={18}/></button></div>
          <form onSubmit={save} className="p-6 space-y-4">
            ...
            <div><label className="block text-sm font-medium mb-1.5">Assigned Engineer *</label>
              <select className={iCls} value={form.assignedEngineer} onChange={u('assignedEngineer')} required><option value="">Select engineer…</option>{engineers.map(e=><option key={e._id} value={e._id}>{e.firstName} {e.lastName}</option>)}</select></div>
            <div><label className="block text-sm font-medium mb-1.5">Order Type *</label>
              <select className={iCls} value={form.typeOfOrder} onChange={u('typeOfOrder')} required><option value="">Select…</option>{ORDER_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
            <div><label className="block text-sm font-medium mb-1.5">Issue Description *</label><textarea className={iCls} rows={3} value={form.issue} onChange={u('issue')} required/></div>
            <div className="grid grid-cols-3 gap-4">
              <div><label className="block text-sm font-medium mb-1.5">Site Visit</label><input className={iCls} type="date" value={form.visitSiteDate} onChange={u('visitSiteDate')}/></div>
              <div><label className="block text-sm font-medium mb-1.5">Start Date</label><input className={iCls} type="date" value={form.startMaintenanceDate} onChange={u('startMaintenanceDate')}/></div>
              <div><label className="block text-sm font-medium mb-1.5">End Date</label><input className={iCls} type="date" value={form.endMaintenanceDate} onChange={u('endMaintenanceDate')}/></div>
            </div>
            <div><label className="block text-sm font-medium mb-1.5">Punctuality</label>
              <select className={iCls} value={form.punctuality} onChange={u('punctuality')}><option value="">Not set</option>{PUNCTUALITIES.map(p=><option key={p}>{p}</option>)}</select></div>
