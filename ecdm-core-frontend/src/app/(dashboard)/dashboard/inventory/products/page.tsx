...
export default function ProductsPage() {
    ...
    return (<div className="space-y-6">
        <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold flex items-center gap-3"><Package size={24} className="text-indigo-400" /> Products</h1><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Product catalog & stock levels ({total} total)</p></div>
            <button onClick={openC} className="flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-all"><Plus size={16} /> Add Product</button>
        </div>
        <div className="flex gap-3 flex-wrap items-center">
            <div className="relative w-64"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" /><input type="text" placeholder="Search products..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className={cn(inputCls, 'pl-10')} /></div>
            <button onClick={() => { setLowStock(!lowStock); setPage(1); }} className={cn('flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all', lowStock ? 'bg-amber-500/15 border-amber-500/30 text-amber-400' : 'border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--secondary))]')}><AlertTriangle size={14} /> Low Stock</button>
        </div>

        <div className="glass-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-[hsl(var(--border))] text-left text-[hsl(var(--muted-foreground))]"><th className="px-4 py-3 font-medium">SKU</th><th className="px-4 py-3 font-medium">Product</th><th className="px-4 py-3 font-medium">Category</th><th className="px-4 py-3 font-medium text-right">Price</th><th className="px-4 py-3 font-medium text-right">Stock</th><th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3 font-medium text-right">Actions</th></tr></thead>
            <tbody>{loading ? <tr><td colSpan={7} className="px-4 py-12 text-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-[hsl(var(--primary))]/30 border-t-[hsl(var(--primary))] mx-auto" /></td></tr>
                : rows.length === 0 ? <tr><td colSpan={7} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">No products found.</td></tr>
                    : rows.map(p => <tr key={p._id} className="border-b border-[hsl(var(--border))]/50 hover:bg-[hsl(var(--secondary))]/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right">{money(p.unitPrice)}</td>
        ...
        {modal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setModal(false)}><div className="glass-card w-full max-w-lg p-6 animate-fade-in bg-[hsl(var(--card))] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6"><h3 className="text-lg font-bold">{editing ? 'Edit Product' : 'New Product'}</h3><button onClick={() => setModal(false)} className="rounded-lg p-1 hover:bg-[hsl(var(--secondary))]"><X size={18} /></button></div>
            <form onSubmit={save} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">SKU *</label><input value={form.sku} onChange={update('sku')} required className={cn(inputCls, 'uppercase font-mono')} placeholder="PROD-001" /></div>
                    <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Category</label><select value={form.category} onChange={update('category')} className={inputCls}><option value="">— None —</option>{cats.map(c => <option key={c._id} value={c._id}>{c.name}</option>)}</select></div>
                </div>
                <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Name *</label><input value={form.name} onChange={update('name')} required className={inputCls} placeholder="Widget Pro 3000" /></div>
                <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Description</label><textarea value={form.description} onChange={update('description')} rows={2} className={cn(inputCls, 'resize-none')} placeholder="Product details..." /></div>
                <div className="grid grid-cols-3 gap-3">
                    <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Unit Price *</label><input type="number" value={form.unitPrice} onChange={update('unitPrice')} required min={0} step={0.01} className={inputCls} placeholder="99.99" /></div>
                    <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Cost Price</label><input type="number" value={form.costPrice} onChange={update('costPrice')} min={0} step={0.01} className={inputCls} placeholder="50.00" /></div>
                    <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Unit</label><input value={form.unit} onChange={update('unit')} className={inputCls} placeholder="pcs" /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                    {!editing && <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Initial Stock</label><input type="number" value={form.currentStock} onChange={update('currentStock')} min={0} className={inputCls} /></div>}
                    <div><label className="mb-1.5 block text-sm font-medium text-[hsl(var(--muted-foreground))]">Low Stock Alert</label><input type="number" value={form.lowStockThreshold} onChange={update('lowStockThreshold')} min={0} className={inputCls} /></div>
                </div>
        ...
        {delId && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setDelId(null)}><div className="glass-card w-full max-w-sm p-6 animate-fade-in bg-[hsl(var(--card))]" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-lg font-bold text-center">Delete Product?</h3><p className="text-sm text-[hsl(var(--muted-foreground))] text-center mt-2">This cannot be undone.</p>
