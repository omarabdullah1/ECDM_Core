    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold">{t.pages.contentTracker.title}</h1><p className="text-sm text-[hsl(var(--muted-foreground))]">{total} {t.common.total}</p></div>
        </div>
        <button onClick={openC} className="flex items-center gap-2 bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] px-4 py-2 rounded-xl text-sm font-medium hover:opacity-90"><Plus size={16}/>{t.pages.contentTracker.addBtn}</button>
      </div>

      <div className="flex gap-3 flex-wrap">
        <select value={fStatus} onChange={e=>{setFStatus(e.target.value);setPage(1);}} className={iCls+' !w-44'}><option value="">{t.common.allStatuses}</option>{STATUSES.map(s=><option key={s}>{s}</option>)}</select>
        <select value={fType} onChange={e=>{setFType(e.target.value);setPage(1);}} className={iCls+' !w-44'}><option value="">{t.common.allTypes}</option>{TYPES.map(tp=><option key={tp}>{tp}</option>)}</select>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]/30">
              <tr>{[t.common.name, t.common.type, t.common.status, t.common.sector, t.common.postDate, t.common.notes, t.common.actions].map(h=><th key={h} className="px-4 py-3 text-start font-medium text-[hsl(var(--muted-foreground))]">{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading?(<tr><td colSpan={7} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">{t.common.loading}</td></tr>)
              :rows.length===0?(<tr><td colSpan={7} className="px-4 py-12 text-center text-[hsl(var(--muted-foreground))]">{t.pages.contentTracker.emptyState}</td></tr>)
              :rows.map(r=>(
      ...
      {modal&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between p-6 border-b border-[hsl(var(--border))]"><h2 className="text-lg font-semibold">{editing ? t.common.edit : t.pages.contentTracker.addBtn}</h2><button onClick={()=>setModal(false)} className="p-2 hover:bg-[hsl(var(--muted))] rounded-lg"><X size={18}/></button></div>
          <form onSubmit={save} className="p-6 space-y-4">
            {error&&<div className="bg-red-500/20 text-red-400 px-4 py-3 rounded-xl text-sm">{error}</div>}
            <div><label className="block text-sm font-medium mb-1.5">{t.common.name} *</label><input className={iCls} value={form.name} onChange={u('name')} required/></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium mb-1.5">{t.common.type} *</label><select className={iCls} value={form.type} onChange={u('type')} required><option value="">{t.common.select}</option>{TYPES.map(tp=><option key={tp}>{tp}</option>)}</select></div>
              <div><label className="block text-sm font-medium mb-1.5">{t.common.status} *</label><select className={iCls} value={form.status} onChange={u('status')} required><option value="">{t.common.select}</option>{STATUSES.map(s=><option key={s}>{s}</option>)}</select></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium mb-1.5">{t.common.sector}</label><select className={iCls} value={form.sector} onChange={u('sector')}><option value="">{t.common.none}</option>{SECTORS.map(s=><option key={s}>{s}</option>)}</select></div>
              <div><label className="block text-sm font-medium mb-1.5">{t.common.postDate}</label><input className={iCls} type="date" value={form.postDate} onChange={u('postDate')}/></div>
            </div>
      ...
      {delId&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] w-full max-w-sm p-6 space-y-4">
          <h2 className="text-lg font-semibold">{t.pages.contentTracker.deleteTitle}</h2>
