...
export default function EditSparePartDialog({ sparePart, onClose, onSuccess }: EditSparePartDialogProps) {
    ...
    const form = useForm<FormSchema>({
        ...
        defaultValues: {
            ...
            unitPrice: String(sparePart.unitPrice || 0),
    ...
    const onSubmit = async (values: FormSchema) => {
        ...
        try {
            ...
            if (dataSheetFile) {
                ...
                console.log('📎 Attaching new file:', dataSheetFile.name);
            }
            // Note: If no new file is uploaded, existing file is preserved server-side
            ...
            const message = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to update spare part';
            toast.error(message);
        } finally {
            setSaving(false);
        }
                                ...
                                <div className="mb-3 p-3 rounded-xl bg-[hsl(var(--muted))]/30 border border-[hsl(var(--border))]">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm font-medium">{sparePart.dataSheetFileName || 'Current Data Sheet'}</p>
                                            <p className="text-xs text-[hsl(var(--muted-foreground))]">Currently attached</p>
                                        </div>
                                        <a
                                            href={getFileUrl(sparePart.dataSheetUrl)}
                                            target="_blank"
                                            rel="noreferrer"
                                            className="p-2 hover:bg-[hsl(var(--muted))] rounded-lg transition-colors"
                                            title="View"
                                ...
                                {dataSheetFile ? (
                                    <div className="flex items-center justify-center gap-3">
                                        ...
                                        <div className="text-left">
                                            ...
                                            <p className="text-xs text-[hsl(var(--muted-foreground))]">
                                                {(dataSheetFile.size / 1024).toFixed(2)} KB (new file)
                                            </p>
                                        </div>
                                        <button
                                            type="button"
                                    ...
                                    <label className="cursor-pointer">
                                        ...
                                        <p className="text-sm text-[hsl(var(--muted-foreground))]">
                                            {sparePart.dataSheetUrl ? 'Upload a new file to replace' : 'Click to upload or drag and drop'}
                                        </p>
